#!/usr/bin/env python3
"""
RES-283 FULL VERSION
- 100 CPPNs (complete dataset for variance analysis)
- 3 Stage 1 budgets: [50, 100, 150]
- 2 strategies: ['uniform', 'decay']
- 32 parallel workers (n2-standard-32)
- Total: ~1.5-2 hours runtime, 200 data points for R² analysis

Use this for final paper results.
"""
...
class ExperimentConfig:
    n_cppns: int = 100                      # FULL: 100 CPPNs
    ...
    stage1_budgets: list = None             # [50, 100, 150] for full analysis
    strategies: list = None                 # ['uniform', 'decay'] both strategies
    n_workers: int = 32                     # Full parallelization on n2-standard-32
...
def run_baseline_single_stage(target_order, image_size, n_live, max_iterations) -> Dict:
    ...
    for iteration in range(max_iterations):
        ...
        if best_order >= target_order and samples_to_target is None:
            # FIXED: Correct sample counting (init samples + 1 per iteration)
...
def run_two_stage_sampling(target_order, image_size, stage1_budget, max_iterations_stage2, strategy='uniform', pca_components=3) -> Dict:
    ...
    # budget=50 → 0 extra iterations, budget=100 → 50 iterations, budget=150 → 100 iterations
    ...
    if pca_mean is not None:
        for iteration in range(max_iterations_stage2):
            ...
            sigma_scale = 0.5 # Default for uniform
            if strategy == 'decay':
                # Decay perturbation magnitude over iterations
                # Start at 0.5, decay to 0.1
                progress = iteration / max_iterations_stage2
                sigma_scale = 0.5 * (1.0 - 0.8 * progress)

            delta_coeffs = np.random.randn(len(coeffs)) * PRIOR_SIGMA * sigma_scale
...
def run_parallel_experiments(config: ExperimentConfig) -> List[Dict]:
    logger.info(f"FULL RUN: {config.n_cppns} CPPNs, {len(config.stage1_budgets)} budgets, {len(config.strategies)} strategies")
...
def analyze_variance(results: List[Dict], config: ExperimentConfig) -> Dict:
    ...
    for result in results:
        ...
        for variant_key, variant_result in result['variants'].items():
            ...
            variant_details.append({
                ...
                'strategy': strategy,
                'speedup': speedup,
                'stage1_samples': variant_result.get('stage1_samples', None),
                'stage2_samples': variant_result.get('stage2_samples', None),
                'total_samples': variant_result.get('total_samples', None)
            })
    ...
    X = np.column_stack([
        stage1_budgets_data,    # Feature 1: Stage 1 budget (50, 100, 150)
        strategy_data,          # Feature 2: Strategy (0=uniform, 1=decay)
        cppn_ids                # Feature 3: CPPN ID (0-99) - captures intrinsic CPPN difficulty
    ])
    y = np.array(speedups)
    ...
    return {
        ...
        'model_details': {
            ...
            'n_samples': X.shape[0],
            'feature_names': ['stage1_budget', 'strategy_decay', 'cppn_id']
        },
        'speedups': speedups,
        'best_speedups': best_speedups,
        'variant_details': variant_details[:20]  # Save first 20 for inspection
    }


def main():
    print("=" * 80)
    print("RES-283 FULL VERSION (100 CPPNs, ~1.5-2 hours runtime)")
    print("=" * 80)
    ...
    try:
        logger.info("[1/3] Running full experiments...")
        results = run_parallel_experiments(config)

        logger.info("[2/3] Analyzing variance...")
        ...
        with open(results_dir / "res_283_full_results.json", 'w') as f:
            json.dump({'config': {'n_cppns': config.n_cppns}, 'analysis': analysis}, f, indent=2)

        print("\n" + "=" * 80)
        print("FULL RES-283 RESULTS")
        print("=" * 80)
        ...
        print(f"Data points: {len(analysis['speedups'])}")
        print(f"\n✓ Full RES-283 complete! Results ready for paper.")
