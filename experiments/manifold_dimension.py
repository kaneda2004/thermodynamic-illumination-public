"""
Manifold Dimension in Feature Space Experiment

Hypothesis: CPPN samples lie on a lower-dimensional manifold in
{density, edge_density, connected_components} feature space than
compressibility-matched random samples.

This tests whether CPPN's efficiency comes from geometric structure
beyond mere compressibility.
...
def generate_random_samples(n_samples: int, image_size: int = 32, seed: int = 123) -> tuple:
    """Generate random binary samples and their features/compressibilities."""
    np.random.seed(seed)
    ...
    for i in range(n_samples):
        # Random binary image with varying density
        density = np.random.uniform(0.1, 0.9)
        img = (np.random.random((image_size, image_size)) < density).astype(np.uint8)
        images.append(img)
        features.append(extract_features(img))
        compressibilities.append(compute_compressibility(img))
...
def run_experiment(
    ...
    """
    Run the manifold dimension experiment.
    ...
    print("=" * 60)
    print("MANIFOLD DIMENSION EXPERIMENT")
    print("=" * 60)
    ...
    # KS test for compressibility match quality
    matched_cppn_comp = np.array([compute_compressibility(img) for img in cppn_imgs[:n_matched]])
    ks_stat, ks_p = stats.ks_2samp(matched_cppn_comp, rand_comp[:n_matched])
    print(f"   KS test for compressibility match: stat={ks_stat:.3f}, p={ks_p:.3f}")
    ...
    # Compile results
    results = {
        'experiment': 'manifold_dimension_orthogonal_features',
        'hypothesis': 'CPPN samples lie on lower-dimensional manifold in feature space',
        'status': status,
        'n_matched': n_matched,
        ...
        'validation_checks': {
            'ks_compressibility_p': float(ks_p),
            'match_rate': float(n_matched / n_samples_cppn),
            'stable_across_k': stable
        },
        'success_criteria': {
    ...
    # Save results
    results_dir = Path(__file__).parent.parent / 'results' / 'manifold_dimension'
    results_dir.mkdir(parents=True, exist_ok=True)

    with open(results_dir / 'dimension_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n   Results saved to: {results_dir / 'dimension_results.json'}")
...
if __name__ == "__main__":
    ...
    print("\n" + json.dumps(results['primary'], indent=2))
