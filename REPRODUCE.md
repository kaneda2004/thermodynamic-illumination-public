...
**Status**: ✅ COMPLETE (5/5 branches completed: 3 validated, 1 refuted, 1 deprecated exploratory)
...
### RES-277: Deprecated Exploratory ViT Sweep

**Status**: Deprecated. This script is retained for historical provenance only and is not claim-grade evidence for the submission.
...
**Issue**: The original script labels an Adam/AdamW axis without instantiating optimizers and does not provide a trustworthy pooled robustness result.

**Do not use RES-277 to support**:
- optimizer robustness
- "14/14 variants broken"
- universal ViT failure claims

**Use instead for submission-facing ViT claims**:
- `RES-358` and `RES-371` for the matched coord-input ConvNet/MLP/ViT comparison
- `RES-357` and `RES-370` for coord-input ViT diagnostics under explicit priors
- `RES-304` and `RES-338` for latent-decoder ViT weakness under the tested priors
...
| Retire unsupported RES-277 claim | Remove from submission-facing evidence | Deprecated and replaced | ✅ PASS |
...
```bash
# Run the 4 claim-grade Phase 0 experiments (total ~3 hours)
uv run python experiments/res_270_tail_mass_sanity_check.py
uv run python experiments/res_276_initialization_ablation.py
uv run python experiments/res_275_order_metric_robustness.py
uv run python experiments/res_279_tail_mass_viz.py
...
- ✓ RES-277 treated as deprecated exploratory evidence
