...
\textbf{Threshold Robustness Validation.} A potential concern is that our findings depend on the specific choice of order threshold $\tau=0.1$. We validate robustness by computing thermodynamic volume across a 10$\times$ range of thresholds from $\tau=0.05$ (permissive) to $\tau=0.5$ (stringent). Table~\ref{tab:mc_volume_thresholds} shows that coarse architecture separation remains consistent: structured priors (FourierBasis, CPPN, ConvDecoder) maintain separation from unstructured priors (MLP, ViT, LSTM) at all thresholds, even though exact within-family ordering can change at stringent thresholds (e.g., WalkCarver versus FourierBasis at $\tau=0.5$). Even at the extreme $\tau=0.5$, FourierBasis achieves 87.2\% while MLP/ViT remain at 0.0\%---confirming our conclusions are not artifacts of threshold choice.

\begin{table}[H]
\centering
\caption{Thermodynamic volume at multiple order thresholds (10,000 samples per architecture). Coarse separation remains stable across a 10$\times$ threshold range, even though some fine ordering changes at stringent thresholds.}
...
\end{tabular}
\end{table}
{\footnotesize For near-zero architectures, std/quantiles are omitted when values collapse to numerically tiny ranges; the mean/max columns already indicate the effective scale.}

\subsection{Initialization Ablation on Coord Inputs}
\label{app:init_ablation}

To test initialization sensitivity, we ran a targeted ablation on coord-input architectures (CoordConvNet, CoordMLP, CoordViT) at 32$\times$32 with $\tau=0.1$ (RES-370). We compare three initialization schemes: fixed Gaussian $\mathcal{N}(0, 0.5)$ in this ablation, Xavier, and He. This ablation therefore uses a different Gaussian scale from the main coord-input comparison, which fixes $\mathcal{N}(0, 0.3)$; the CoordViT Gaussian baseline here (0.335) is not directly comparable to the 3\% baseline reported in the main text. ConvNet and MLP pass rates shift modestly across schemes (ConvNet: 0.665--0.725; MLP: 0.385--0.435). CoordViT is markedly more sensitive: pass rate rises from 0.335 (Gaussian) to 0.855 (Xavier) and 0.725 (He). This sensitivity motivates treating initialization as part of the prior and holding it fixed for all main comparisons.

\subsection{Scaled-Prior Exact Monte Carlo Rerun}
\label{app:scaled_prior}

To test whether the coord-input ConvNet--ViT gap persists under fan-in-scaled priors, we reran the matched $32\times 32$ coordinate trio (CoordConvNet, CoordMLP, CoordViT) with exact Monte Carlo at $\tau=0.1$ (RES-382). Each condition uses 5,000 direct prior samples under one of three priors: the main text's fixed isotropic Gaussian $\mathcal{N}(0, 0.3)$, Xavier, and He.

\begin{table}[H]
\centering
\caption{RES-382 exact Monte Carlo rerun for the matched coord-input trio (5,000 samples per architecture/prior). Pass rate is the direct tail mass estimate $\Pr[O(x)\geq 0.1]$; mean order summarizes average structure under the same prior.}
\label{tab:scaled_prior_rerun}
\begin{tabular}{llcc}
\toprule
Prior & Architecture & Pass rate & Mean order \\
\midrule
\(\mathcal{N}(0,0.3)\) & CoordConvNet & 63.0\% & 0.238 \\
\(\mathcal{N}(0,0.3)\) & CoordMLP & 26.8\% & 0.068 \\
\(\mathcal{N}(0,0.3)\) & CoordViT & 2.9\% & 0.020 \\
\midrule
Xavier & CoordConvNet & 66.8\% & 0.261 \\
Xavier & CoordMLP & 42.9\% & 0.124 \\
Xavier & CoordViT & 85.1\% & 0.527 \\
\midrule
He & CoordConvNet & 67.2\% & 0.262 \\
He & CoordMLP & 42.2\% & 0.120 \\
He & CoordViT & 59.9\% & 0.399 \\
\bottomrule
\end{tabular}
\end{table}

Under the fixed $\mathcal{N}(0,0.3)$ prior, CoordConvNet strongly dominates CoordViT, reproducing the main paper's low-pass Gaussian result. Under Xavier, the ranking flips: CoordViT exceeds CoordConvNet on both pass fraction and mean order. Under He, CoordViT remains below CoordConvNet on pass fraction but exceeds it on mean order. The main conclusion is therefore not that attention is uniformly weak, but that CoordViT is strongly prior-sensitive while CoordConvNet is comparatively robust across the tested priors.

We also logged first-layer attention diagnostics for CoordViT to test whether the low-pass Gaussian result is explained by trivial softmax collapse. In the sampled $\mathcal{N}(0,0.3)$ initializations, the mean maximum absolute attention logit is 12.26 (sample max 19.27), the mean peak attention weight is 0.247, and the exact-zero attention fraction is 0.0 across all logged samples. Under Xavier, the corresponding values are 1.72, 0.0038, and 0.0; under He, 17.96, 0.397, and 0.0. These diagnostics show no exact softmax underflow in the sampled Gaussian(0,0.3) ViT initializations. The low thermodynamic volume under the fixed Gaussian prior is therefore better interpreted as prior sensitivity than as a trivial dead-softmax failure mode.
