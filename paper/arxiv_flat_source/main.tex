...
\begin{enumerate}
    ...
    \item \textbf{Task-specific predictive scope}: $B_{\text{NS}}^{\text{cross}}$ predicts reconstruction but not random-feature classification on our two tested datasets, clarifying that the metric captures generative priors rather than a universal notion of architecture quality.
...
The discovery that untrained CNNs serve as powerful image priors \cite{ulyanov2018deep} sparked interest in understanding architecture-induced biases. Subsequent work showed that architectural choices such as upsampling and spectral bias strongly influence the prior \cite{liu2023devil,shi2022spectral}. However, these works demonstrate bias qualitatively through reconstruction examples or task-specific denoising ablations. Our framework provides, to our knowledge, the first \emph{quantitative thermodynamic comparison} of structured-image event mass under explicit random-weight priors, enabling statements like ``uniform priors require at least 70 additional NS crossing bits ($\ge 2^{70}$ protocol depth factor) to match CPPN under a fixed nested-sampling protocol''---a nominal crossing-depth factor, not a calibrated tail-probability ratio.
...
Mellor et al.\ \cite{mellor2021neural} introduced metrics to predict trained accuracy from untrained network statistics, enabling neural architecture search without training. Their metrics examine activation patterns and gradient flow. Our key finding is that such predictions are \emph{task-dependent}: bits predicts reconstruction but shows no significant relationship with classification on our tested datasets. This task specificity is invisible to activation-based metrics that aim at a single architecture-quality score.
...
\small
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}p{3.7cm}p{3.3cm}p{3.6cm}p{3.3cm}@{}}
\toprule
...
The multiplicative form ensures \emph{all} properties must be present---a highly compressible but disconnected image scores low, as does a symmetric but imbalanced image. Note that this acts as a strict logical AND gate: it captures a specific, stringent definition of compositional geometric structure, so an image with a highly regular, compressible pattern but no bilateral symmetry will still score near zero.
...
\small
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}p{5.0cm}p{3.4cm}p{5.4cm}@{}}
\toprule
...
5.8 Task Specificity & 28$\times$28 grayscale$^\ddagger$ & Derived from Sections 5.6--5.7 \\
...
\small
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}p{4.8cm}p{1.8cm}p{2.1cm}p{2.4cm}p{3.1cm}@{}}
\toprule
...
\textbf{Controlled Comparison: CoordConvNet vs CoordViT.} To probe locality with weight sharing versus global mixing under a fixed matched-input prior, we compare architectures receiving \emph{identical} coordinate grid inputs $(x,y) \in [-1,1]^2$ at 32$\times$32 resolution. CoordConvNet processes the coordinate grid with 3$\times$3 convolutions (10.1K params); CoordViT processes the same grid with global self-attention (17.2K params); CoordMLP serves as an intermediate baseline (8.6K params). Using nested sampling (100 live points, 2000 iterations):
...
In the DIP validation experiments, transformer-based models use standard framework initialization rather than the fixed isotropic Gaussian prior used in the coord-input study; in the 13-architecture reconstruction sweep, positional embeddings are additionally initialized at small scale. The observed failures are therefore not attributable to trivial dead-softmax saturation or dead-gradient failure in the tested setups.
...
\begin{enumerate}
    ...
    \item \textbf{The Broken Regime} (Global ViT, Hybrid ViT; structure $\approx 0$, PSNR $\approx 10$dB): Global attention at random initialization creates an untrained optimization-failure regime under this protocol; these architectures fail to fit even the noisy target reliably.
\end{enumerate}
...
\textbf{Implication.} The three-regime taxonomy offers concrete guidance within our untrained generative setting: to \emph{generalize} in DIP-style reconstruction, choose Shielded architectures; to avoid reconstruction failure, avoid global attention at random initialization for pixel-space objectives. Structure predicts \emph{generalization gap} in this setting, suggesting a quantitative pre-screening signal rather than a universal architecture-selection rule. A permutation test (10,000 permutations) confirms the correlation is not spurious: only 21 random shuffles achieved $|\rho| \geq 0.79$, yielding empirical $p = 0.002$.
...
\textbf{Validation of Nested Sampling.} \emph{Critical note on calibration:} Monte Carlo (64$\times$64) and nested sampling (32$\times$32) use different methodologies, making absolute bit values incomparable across methods. MC directly counts successes ($\text{bits} = -\log_2(\text{fraction})$); NS estimates volume via iterative compression with different calibration. What IS validated is the \textbf{coarse architecture separation}: both methods place strongly structured priors (WalkCarver/FourierBasis/FourierMLP/CPPN/ConvDecoder) well above broken priors (ViT/MLP-style families) across all thresholds tested ($\tau = 0.05$ to $0.5$), even though exact within-family ordering can change at stringent thresholds. This coarse consistency across methods, resolutions, and thresholds validates the thermodynamic interpretation. (For resolution scaling \emph{within} a single method, see Section~\ref{sec:limitations}: bits increase sub-linearly with resolution.)
...
\textbf{Results.} Figure~\ref{fig:volume} shows the order metric versus explored prior depth ($-\log_2 X$), juxtaposing continuous CPPN/Uniform trajectories with threshold traces for coordinate-conditioned ConvNet/MLP/ViT as anchor examples from separate NS protocols. Figure~\ref{fig:volume_zoomed} zooms in on the matched coordinate comparison over the first 15 bits, highlighting early structure emergence within that fixed protocol.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{fig1_volume_comparison.pdf}
\caption{Volume comparison: order metric vs NS depth ($-\log_2 X$). Continuous curves show CPPN and Uniform baselines; dashed traces show coordinate-conditioned architectures (CoordConvNet/CoordMLP/CoordViT). These traces are shown together as anchor examples from separate protocols for visual context; raw bit heights should not be read as directly comparable across protocols. The dashed horizontal line marks $\tau = 0.1$.}
\label{fig:volume}
\end{figure}
...
\caption{NS threshold-crossing bits to reach $\tau = 0.1$ for key anchors. CoordConvNet/CoordMLP/CoordViT come from the matched-input coordinate comparison; CPPN/Uniform come from separate prior-comparison baselines. Bars are shown together as protocol-specific anchors rather than directly comparable raw bit values across experimental families. Lower is better within each fixed protocol.}
\label{fig:bits}
\end{figure}
...
\caption{MNIST random-feature linear probe accuracy vs $B_{\text{NS}}^{\text{cross}}(\tau{=}0.1)$ shows no significant correlation. This is an important negative result: the bits metric that predicts reconstruction does \emph{not} predict random-feature classification. Table~\ref{tab:classification} reports both MNIST and FashionMNIST.}
...
\subsection{Task Specificity of the Bits Metric}

Why does bits predict reconstruction but not classification? Our results support \textbf{task specificity} rather than a validated inverse trade-off. In the tested random-feature setting, low-$B_{\text{NS}}^{\text{cross}}$ architectures are better generative priors, but we do not establish a monotonic inverse relationship between reconstruction and classification performance.

A plausible hypothesis is a \textbf{generative-discriminative tension}:

\textbf{Low bits (CPPN-like):} Strong topology preservation. Nearby latent codes map to similar images. This is ideal for reconstruction (smooth latent space) but may blur class boundaries in a random-feature classifier.

\textbf{High bits (MLP-like):} Random projection behavior \cite{johnson1984extensions}. Distances are preserved but topology is destroyed. This may help discrimination while harming reconstruction by removing smoothness priors.

We leave this mechanism as a hypothesis for future work rather than a claimed result from the current classification experiments.
...
\paragraph{Additional ViT Diagnostics.} Separate supporting checks under explicit untrained priors also leave ViT families weak, but we treat them as diagnostics rather than a single pooled robustness sweep. In coord-input ablations, varying patch size, positional encoding, width, and heads leaves ViT below the ConvNet baseline under our fixed isotropic Gaussian prior. Changing the initialization scheme, however, reveals strong prior sensitivity: in an exact Monte Carlo rerun, CoordViT pass fraction rises from 2.9\% under the fixed Gaussian prior to 85.1\% under Xavier initialization, exceeding the ConvNet baseline (66.8\%), while He narrows but does not reverse the pass-rate ranking (Appendix~\ref{app:scaled_prior}). In latent-decoder checks, ViT variants remain in the low-order regime. These results support the narrower claim that several ViT families remain weak under the tested priors, without implying optimizer-invariant or universal failure.
...
\paragraph{Threshold Robustness.} Coarse architecture separation is stable across a wide range of order thresholds. We validate at $\tau \in \{0.05, 0.1, 0.2, 0.3, 0.5\}$ with 10,000 samples per architecture (Appendix Table~\ref{tab:mc_volume_thresholds}). Even at the extreme threshold $\tau=0.5$, separation is maintained: FourierBasis achieves 87.2\%, CPPN 12.4\%, while MLP/ViT remain at 0.0\%. This confirms our findings are not artifacts of threshold choice, even though exact within-family ordering can change at stringent thresholds.
...
Additional ViT diagnostics & Weak across tested explicit-prior setups & \checkmark \\
...
This work provides, to our knowledge, the first quantitative thermodynamic comparison of image-generation priors under explicit random-weight protocols. Previous work demonstrated that architecture matters \cite{ulyanov2018deep} or evolved architectures for specific tasks \cite{gaier2019weight}, but could not answer ``how much event mass does architecture X allocate to structured outputs compared to architecture Y under a fixed prior?'' Our bits metric answers this directly.
...
Beyond quantitative comparison, our work provides \emph{setup-specific mechanistic understanding} rather than a single universal mechanism. In the matched coordinate-conditioned trio, locality with weight sharing is the strongest driver of low crossing cost. In the matched-decoder study, decoder choice is the primary bottleneck determining whether structure reaches the output. Within the CPPN family, compositional coordinate structure appears to shape manifold dimensionality, while activation choice affects absolute order magnitude. These mechanisms are complementary rather than contradictory: they operate in different experimental families under explicitly specified priors. The observed $\sim$2--4$\times$ mean speedup under non-trivial targets indicates that this geometric structure is operationally exploitable, though gains are noisy and not easily predicted.
...
Our results suggest scoped guidance for the untrained generative settings studied here:

\begin{itemize}
    \item \textbf{For DIP-style reconstruction tasks} (denoising, inpainting, super-resolution under untrained priors): the low-$B_{\text{NS}}^{\text{cross}}$ architectures in our experiments, such as CPPNs and deep convolutions, provide stronger implicit regularization and better reconstructions.

    \item \textbf{For structured image-generation settings where target statistics are known}: matching architecture bias to the target distribution is a useful hypothesis to test (Section~\ref{sec:alignment}). Fourier Features ($0.54$) closely match natural images ($0.53$), consistent with their empirical efficacy in NeRF.
\end{itemize}

\paragraph{Exploratory prospective screening.} In additional held-out DIP-style denoising sweeps with frozen rankings (three large target-seed replications; 20 targets, 5 optimization seeds, 1000 steps), a simple bits-only ranking achieved a top-2 hit rate of $0.87 \pm 0.03$ and mean regret of $0.13 \pm 0.04$ dB across replications. Handcrafted conditional refinements did not improve on this baseline at the same scale. We therefore interpret thermodynamic bits as a promising \emph{shortlist signal} for the tested untrained inverse setting, not as a solved zero-shot NAS rule.
...
Our framework provides a thermodynamic lens on the observation that Vision Transformers often require more training data than ConvNets \cite{steiner2022how}. We find that untrained ViTs with standard patch-wise decoders occupy the same high-entropy region as MLPs, despite having positional embeddings (Section~\ref{sec:spectrum64}). The mechanism is setup-dependent rather than singular: in the matched-decoder experiment, the patch-based decoder is the primary bottleneck, since replacing it with a convolutional upsampler moves ViT from Broken to Shielded (Section~\ref{sec:bias-chain}); in the matched coordinate-conditioned trio, a gap remains even with identical inputs, indicating that global mixing without weight sharing is itself less structure-promoting than convolution. We therefore interpret decoder choice and mixing pattern as complementary contributors whose relative importance depends on the comparison.
...
\paragraph{Controlled Comparison with Matched Inputs.} To probe locality with weight sharing versus global mixing under a fixed matched-input prior, we test coordinate-conditioned architectures receiving identical $(x,y)$ coordinate inputs (Section~\ref{sec:spectrum64}). CoordConvNet (3$\times$3 convolutions) achieves $B_{\text{NS}}^{\text{cross}}=0.72$ bits with 64\% initial live-point pass fraction; CoordMLP (no spatial weight sharing) requires $B_{\text{NS}}^{\text{cross}}=2.11$ bits with 21\%; CoordViT (global attention) requires $B_{\text{NS}}^{\text{cross}}=2.67$ bits with 3\%---a $\sim$2-bit $B_{\text{NS}}^{\text{cross}}$ gap and $\sim$21$\times$ pass-fraction gap between ConvNet and ViT. This controlled design supports attributing the gap, within this protocol, to architectural mixing with or without weight sharing rather than to input representation, decoder architecture, or parameter count.

\paragraph{Prior-conditioned bias.} Inductive bias is strongly prior-conditioned rather than an absolute architectural constant. In our matched coord-input comparison, convolutional structure is comparatively robust across the tested priors, whereas CoordViT is highly sensitive to variance scaling and can become competitive or superior under Xavier-scaled priors (Appendix~\ref{app:scaled_prior}). We therefore interpret the flagship CoordConvNet--CoordViT gap as a property of the fixed isotropic Gaussian prior used in the main comparison, not as a universal statement that attention architectures intrinsically lack structural bias.
...
\paragraph{Metric Specificity.} Our order$_{\text{multiplicative}}$ metric is not intended as a universal structure measure. What the paper establishes is a stable \emph{coarse architecture ranking} across the tested metric families and thresholds (Appendix~\ref{app:robustness}), together with strong agreement to spectral-slope and LPIPS-based validations. What changes across metrics are the absolute scales and, for some orthogonal metrics, which aspect of images is emphasized. The metric is therefore best interpreted as one validated probe of compositional geometric structure rather than a universal score for all downstream tasks.
...
\paragraph{Input distribution specification.} Different architecture families use different input conventions: CPPNs receive fixed coordinate grids, while ConvNets and ViTs receive random noise (Section~\ref{sec:prior_spec}). To test whether this alone confounds our results, we ablated input type: a coordinate-input MLP with ReLU activations (matching ConvNet activations but CPPN inputs) achieves only 15.2\% success rate---\emph{worse} than ConvDecoder with random noise (52.3\%). This rules out coordinate inputs by themselves as the source of CPPN's advantage. Within the CPPN family, the evidence instead suggests a combination of compositional coordinate structure and activation choice: coordinate composition appears to shape manifold dimensionality (Section~\ref{sec:phase_transition}), while periodic activations affect absolute order magnitude. The input specification is therefore not a standalone confound.

\subsection{Future Work}

Our framework opens several promising research directions:

\paragraph{Thermodynamic Neural Architecture Search (T-NAS).} The structure-generalization relationship (Section~\ref{sec:scaling_law}) motivates a possible screening workflow: evaluate untrained priors first, then train only shortlisted architectures. Our exploratory held-out DIP sweeps suggest that a frozen bits-only ranking can already provide useful shortlist quality in the tested untrained inverse setting, while simple handcrafted conditional refinements do not consistently improve that baseline. Broader prospective validation across datasets, corruption regimes, and architecture pools is still required before claiming realized NAS speedups.
...
\begin{enumerate}
    ...
    \item \textbf{Generative-specific metric}: Low-$B_{\text{NS}}^{\text{cross}}$ architectures excel at generation; however, $B_{\text{NS}}^{\text{cross}}$ does \emph{not} predict classification performance in our random-feature probes (MNIST Spearman $\rho=-0.24$, FashionMNIST Spearman $\rho=0.01$, both $p>0.25$). This confirms that our metric captures generative priors specifically.
    \item \textbf{ConvNet-Transformer gap}: With matched coordinate inputs, CoordConvNet ($B_{\text{NS}}^{\text{cross}}=0.72$ bits, 64\% initial live-point pass fraction) and CoordViT ($B_{\text{NS}}^{\text{cross}}=2.67$ bits, 3\%) show a $\sim$2-bit $B_{\text{NS}}^{\text{cross}}$ gap and a $\sim$21$\times$ pass-fraction gap. In this matched coordinate-conditioned setup, the comparison is consistent with locality plus weight sharing being a major source of spatial bias under the fixed prior.
...
This work transforms inductive bias from a qualitative notion (CNNs prefer smooth images) to measurable protocol-specific quantities, including threshold-crossing costs under fixed priors and direct tail-mass estimates where Monte Carlo is feasible. We hope this enables principled comparison of untrained generative priors, provides a scoped shortlist signal for the tested untrained inverse setting, and informs future prospective work on architecture pre-screening and output-space geometry.
