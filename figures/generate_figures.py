...
def fig1_volume_comparison():
    ...
    ax.set_xlabel("Bits of Prior Volume Explored")
...
def fig_threshold_curve():
    ...
    ax.text(
        0.28,
        1.05,
        f"Delta at tau=0.1: {vit_bits - conv_bits:.2f} bits",
        fontsize=9,
        bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="none", alpha=0.85),
    )

    ax.set_xlabel("Order Threshold tau")
    ax.set_ylabel("Bits Required B(tau)")
...
def fig3_reconstruction_correlation():
    ...
    ax.set_xlabel("Prior Bits (lower = more structured)")
    ax.set_ylabel("Reconstruction MSE (lower = better)")
    ax.set_title(f"Reconstruction Quality vs Prior Bits\nSpearman r = {rho:.3f}, p = {p_value:.4f}")
    ax.grid(True, alpha=0.3)
...
def fig4_classification_null():
    ...
    ax.set_title(f"Classification vs Prior Bits\nSpearman r = {rho:.3f}, p = {p_value:.3f}")
    ax.grid(True, alpha=0.3)
